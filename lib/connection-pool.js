...
function createConnection(pooled) {
    ...
    self.log(`createConnection:${connection}`);
    try {
    ...
    // self.log(`createConnection:pooled=${JSON.stringify(pooled)}`);
    if (pooled) {
        pooled.id = cid++;
        ...
        // self.log(`createConnection: create pooled=${JSON.stringify(pooled)}`);
        this.connections.push(pooled);
        self.log(`createConnection: create connections=${JSON.stringify(this.connections)}`);
    }
